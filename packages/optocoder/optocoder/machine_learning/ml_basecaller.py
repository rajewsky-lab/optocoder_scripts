...
class ML_Basecaller():
    ...
    def _prepare_data(dataset, num_cycles):
        ...
        for i in range(num_cycles):
            ys[:, i] = le.fit_transform(ys[:, i].ravel()).reshape(ys[:, i].shape)
    ...
    def _predict_rnn(self, xs, ys, model_type='rnn'):
        probabilites = self.classifiers[model_type].predict_proba(xs)
        predictions = self.classifiers[model_type].predict_classes(xs)
    ...
    def predict(self, model_type):
        ...
        if 'rnn' in model_type:
            predictions, probabilites = self._predict_rnn(xs, ys, model_type)
        else:
    ...
    def predict_all(self, model_type):
        """Predict the barcodes for all beads"""
       
        non_matching_ids = self.all_beads[~self.all_beads['bead_id'].isin(self.matching_beads['bead_id'])]['bead_id']
        matching_ids = self.matching_beads['bead_id']

        non_matching_ids = self.all_beads['bead_id']
        dataset = self.all_beads
        xs, ys = self._prepare_data(dataset, self.experiment.num_cycles)
